"""
Oura → Supabase sync
Fetches sleep and readiness scores via the Oura API v2.

Schedule: run once daily (data lags ~1hr after waking)
  30 8 * * * python src/sync_oura.py
"""
...
def fetch_sleep(start_date: str, end_date: str) -> dict:
    resp = requests.get(f"{BASE}/sleep", headers=HEADERS, params={"start_date": start_date, "end_date": end_date})
    resp.raise_for_status()
    return {d["day"]: d for d in resp.json().get("data", [])}


def fetch_readiness(start_date: str, end_date: str) -> dict:
    resp = requests.get(f"{BASE}/readiness", headers=HEADERS, params={"start_date": start_date, "end_date": end_date})
    resp.raise_for_status()
    return {d["day"]: d for d in resp.json().get("data", [])}
...
def upsert_scores(sleep_by_day: dict, readiness_by_day: dict):
    ...
    for day in all_days:
        ...
        rows.append({
            ...
            "temperature_deviation": r.get("contributors", {}).get("body_temperature"),
